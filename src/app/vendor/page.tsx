...
export default function AddProductPage() {
  ...
  // handle upload for main or thumbnails
  const handleImageUpload = async (
    ...
    try {
      // 1. Get signed data from backend
      ...
      // 3. Update UI state with uploaded URL instead of local blob
      if (index === undefined) {
        // main image
        setMainImage(uploadedUrl);
        setMainImageResponse(body);
      } else {
        // push body into the array
        setAdditionalImageResponse((prev) => [...(prev ?? []), body]);
  ...
  "stock_items": data.stock_items
    ...
    try {
      const response = await addProducts(x);
  ...
  return (
    <div className="min-h-screen  p-6">
      <div className="flex justify-between">
        <div>
          <p className="text-sm text-gray-500">PRODUCT LIST/ ADD NEW PRODUCT</p>
        ...
        <div className="flex justify-end gap-4 ">
          <Button
            variant="ghost"
            type="button"
            className="flex items-center gap-2 text-black border border-black rounded-none uppercase w-[167px]"
          >
          ...
          <Button
            ...
            type="submit"
            className="flex items-center gap-2 uppercase rounded-none w-[167px]"
          >
            <Plus className="w-4 h-4" />
            {status === "loading" ? <LoadingDots /> : "Add Product"}
          </Button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl pt-10">
        <form className="grid md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            {/* Main Preview */}
            <div className="h-[536px] w-[564px] bg-grey-light rounded-none flex flex-col items-center justify-center">
              <Input
                type="file"
                className="hidden"
                id="main-upload"
            ...
            <div className="w-[564px] grid grid-cols-4 gap-2">
              {thumbnails.map((img, idx) => (
                <div
                  key={idx}
                  className={`w-[129px] h-[143px] rounded-none border-2 border-dashed flex items-center justify-center overflow-hidden cursor-pointer transition ${
                    img
