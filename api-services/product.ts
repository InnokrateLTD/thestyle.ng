...
export const editProduct = async (id: string, values: RequestParams): Promise<AxiosResponse<ApiResponse<null>>> => {
  const result = await mutationRequest(`${ApiRoutes.SingleProduct}/${id}/update/`, "patch", values);
  return result;
};
...
export const useGetSellerReview = (
  search?: string,
  page: number = 1,
  page_size: number = 10,
  service: string = "style-ng"
) => {
  const url = `products/seller/reviews/?page=${page}&page_size=${page_size}&search=${search}`;
...
export const deleteProduct = async (id: string): Promise<AxiosResponse<ApiResponse<null>>> => {
  const result = await mutationRequest(`${ApiRoutes.SingleProduct}/${id}/delete/`, "delete");
  return result;
};
export const deleteReview = async (id: string): Promise<AxiosResponse<ApiResponse<null>>> => {
  const result = await mutationRequest(`${ApiRoutes.DeleteReview}/${id}/`, "delete");
  return result;
};

export const useGetBestSeller = (
  time_period: string = 'custom',
  end_date: string = '',
  start_date: string = '',
  service: string = "style-ng"
) => {
  const url = `products/best-selling/?time_period=${time_period}&start_date=${start_date}&end_date=${end_date}`;
